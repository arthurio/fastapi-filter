...
@pytest.mark.asyncio
async def test_filter(Address, User, UserFilter, filter_, expected_count):
    query = User.all().select_related('address')
    query = await UserFilter(**filter_).filter(query)
    assert len(query) == expected_count

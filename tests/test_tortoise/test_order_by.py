...
@pytest.mark.asyncio
async def test_order_by(User, UserFilterOrderBy, users, order_by, assert_function):
    query = User.all()
...
@pytest.mark.asyncio
async def test_order_by_with_default(User, UserFilterOrderByWithDefault, users):
    query = User.all()
    query = await UserFilterOrderByWithDefault().sort(query)
    previous_user = None
    for user in query:
        if not previous_user:
            previous_user = user
...
@pytest.mark.asyncio
async def test_api_order_by_with_default(test_client, users, order_by, assert_function):
...
@pytest.mark.asyncio
async def test_custom_order_by(User, users, UserFilterCustomOrderBy, order_by, assert_function):
    query = User.all()
    query = await UserFilterCustomOrderBy(custom_order_by=order_by).sort(query)
    previous_user = None
    for user in query:
        if not previous_user:
            previous_user = user
